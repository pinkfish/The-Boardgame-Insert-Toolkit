...
function
__is_element_isolated_for_print() = g_isolated_print_box != "" && (__index_of_key(data, g_isolated_print_box) != []);
...
function __label_auto_width(label,
                            x,
...
Shear(x, y, height) {
  ...
  multmatrix(m = [ [ 1, 0, sin(x), 0 ], [ 0, 1, sin(y), 0 ], [ 0, 0, 1, 0 ] ]) translate([ 0, 0, -height / 2 ])
  children();
}
...
MakeAll() {
  if (__is_element_isolated_for_print()) {
    ...
    for (i = [0:__num_elements() - 1]) {
      ...
      element_position = (g_b_vis_actual && __box_vis_position(element) != []) ? __box_vis_position(element)
...
MakeDividers(div) {
  ...
  module MakeDivider(title, tab_offset) {
    ...
    difference() {
      ...
      translate(text_pos)
      resize([ text_width, 0, 0 ], auto = [ true, true, false ]) linear_extrude(depth) text(text = title,
                                                                                            font = font,
                                                                                            size = font_size,
                                                                                            valign = "top",
                                                                                            halign = "center",
...
MakeBox(box) {
  ...
  m_wall_thickness =
    g_b_fit_test ? 0.5 : __value(box, "wall_thickness", default = g_wall_thickness); // needs work to change if no lid
  ...
  m_lid_tab_height = __value(m_lid, LID_TAB_HEIGHT, default = 1);

  function __lid_notch_depth() = m_wall_thickness / 2 + m_lid_tab_height-1;
  ...
  m_lid_tab = [ max(m_box_size[k_x] / 4, g_detent_spacing * 2), m_wall_thickness, __lid_external_size(k_z) + m_lid_tab_height ];
  ...
  module MakeLayer(component, layer = "") {
    ...
    echo(
      [ m_component_base_height, m_box_size[k_z], __component_size(k_z), m_wall_thickness, __compartment_size(k_z) ]);
    // DERIVED VARIABLES
    ...
    function __component_size(D) = (D == k_z)
    ...
    module ContainWithinBox() {
      ...
      if (b_needs_trimming && (__component_position(k_x) < m_box_inner_position_min[k_x] ||
        ...
        echo("<br><font color='red'>WARNING: Components in RED do not fit in box. If this is not intentional then " +
             "adjustments are required or pieces won't fit.</font><br>");
      ...
      if (g_print_mmu_layer == "mmu_label_layer") {
        ...
        union() {
          ...
          if (!m_lid_inset && m_box_has_lid) {
            ...
            MirrorAboutPoint(v = [ 0, 0, 1 ], pt = [ 0, 0, __lid_external_size(k_z) / 2 ]) MakeLidBase_Cap();
          ...
          if (m_lid_inset && m_box_is_stackable) {
            difference() {
              ...
              MirrorAboutPoint(v = [ 0, 0, 1 ], pt = [ 0, 0, __lid_external_size(k_z) / 2 ]) MakeLidBase_Inset(
                tolerance = g_tolerance, tolerance_detent_pos = g_tolerance_detent_pos, omit_detents = !m_lid_inset);
            }
          }

          if (m_lid_fit_under && m_box_has_lid) {
            if (m_lid_inset)
              translate([ 0, 0, -__lid_external_size(k_z) + m_lid_tab_height - 1 ]) // move it down
            MakeLidTabs(mod = 0);
            else {
      ...
      difference() {
        ...
        translate([ 0, 0, __lid_external_size(k_z) / 2 ])
        {
          ...
          MirrorAboutPoint(v = [ 0, 1, 0 ],
            ...
            MirrorAboutPoint(v = [ 1, 0, 0 ],
                             pt =
                               [ __lid_external_size(k_x) / 2, __lid_external_size(k_y) / 2, m_lid_wall_height / 2 ]) {
            cube([ __lid_external_size(k_x), __lid_notch_depth() + tolerance, __lid_external_size(k_z) / 1 ]);
            cube([ __lid_notch_depth() + tolerance, __lid_external_size(k_y), __lid_external_size(k_z) / 1 ]);
          }
        }
    ...
    module MakeLidEdge(extra_height = 0, extra_depth = 0, offset = 0) {
      ...
      {
        cube([ m_box_size[k_x] - (2 * offset), __lid_notch_depth() + extra_depth, m_lid_wall_height + extra_height ]);

        cube([ __lid_notch_depth() + extra_depth, m_box_size[k_y] - (2 * offset), m_lid_wall_height + extra_height ]);
      }
    }
    ...
    module MakeBoxShellWithNewLidBits() {
      difference() {
        ...
        translate([ 0, 0, m_box_size[k_z] - m_lid_tab[k_z] ])
    ...
    module InnerLayer() {
      ...
      if (m_is_component_subtractions) {
        ...
        if (m_component_has_side_cutouts) {
          ...
          sizex = __component_size(k_x) - 2 * m_cutout_size_frac_aligned * __compartment_size(k_x) -
                  __component_margins_sum(k_x);
          sizey = __component_size(k_y) - 2 * m_cutout_size_frac_aligned * __compartment_size(k_y) -
    ...
    module MakeLidLabel(label, offset = 0, thickness = m_lid_thickness) {
      ...
      MirrorAboutPoint([ 1, 0, 0 ], [ 0, 0, thickness / 2 ])
        RotateAboutPoint(__label_rotation(label), [ 0, 0, 1 ], [ 0, 0, 0 ]) Make2dLidLabel(label, width, offset);
    }
    ...
    module Helper_MakeBoxLabel(label, x = 0, y = 0) {
      ...
      RotateAboutPoint(__label_rotation(label), [ 0, 1, 0 ], [ 0, 0, 0 ]) RotateAboutPoint(90, [ 1, 0, 0 ], [ 0, 0, 0 ])
    ...
    module MakeLidLabelFrame(label, offset = 0, thickness = m_lid_thickness) {
      ...
      linear_extrude(thickness) translate([ xpos, ypos, 0 ])
      MirrorAboutPoint([ 1, 0, 0 ], [ 0, 0, thickness / 2 ])
        RotateAboutPoint(__label_rotation(label), [ 0, 0, 1 ], [ 0, 0, 0 ]) offset(r = offset) intersection() {
        hull() {
          translate([ -200, 0, 0 ])
    ...
    module MakeDetachedLidLabels() {
      ...
      MoveToLidInterior(tolerance = -g_tolerance) translate(g_b_vis_actual ? lid_vis_position : lid_print_position)
      RotateAboutPoint(g_b_vis_actual ? 180 : 0,
                       [ 0, 1, 0 ],
                       [ __lid_external_size(k_x) / 2, __lid_external_size(k_y) / 2, 0 ]) MakeAllLidLabels();
    }
              ...
              if (m_lid_label_bg_thickness > 0)
                difference() {
                  intersection() {
                    ...
                    translate([ x / 2 - (x2 + offset_thickness) / 2, y / 2 - (y2 + offset_thickness) / 2, 0 ])
                    RotateAboutPoint(
                      theta, [ 0, 0, 1 ], [ (x2 + offset_thickness) / 2, (y2 + offset_thickness) / 2, 0 ])
                      MakeStripedGrid(x = x2 + offset_thickness,
                                      y = y2 + offset_thickness,
                                      w = 0.5,
                                      dx = 1,
                                      dy = 0,
                                      depth_ratio = 0.5,
    ...
    module MakeSideCutouts(side, margin = false) {
      ...
      pos_standard = [
        ...
        // back
        [ __size(k_x) / 2 - perp_size / 2, __size(main_d) - main_size + __padding(k_y) / 2, __finger_cutouts_bottom() ],
        // left
        [ -__padding(k_x) / 2, __size(k_y) / 2 - perp_size / 2, __finger_cutouts_bottom() ],
        // right
        [ __size(main_d) - main_size + __padding(k_x) / 2, __size(k_y) / 2 - perp_size / 2, __finger_cutouts_bottom() ],
      ];

      pos_margin = [
        // front
        [ __size(k_x) / 2 - perp_size / 2, -__padding(main_d) / 2 - g_wall_thickness, __finger_cutouts_bottom() ],
        // back
        [
        ...
        // left
        [ -__padding(main_d) / 2 - g_wall_thickness, __size(k_y) / 2 - perp_size / 2, __finger_cutouts_bottom() ],
        // right
        [
      ...
      shape = __round_bottom() ? [ t, t, t, t ] : m_actually_cutout_the_bottom ? shape_square : shape_standard[side];
    ...
    module AddFillets() {
      ...
      module _MakeFillet() {
        difference() {
          cube_rotated =
            __component_shape_rotated_90() ? [ __compartment_size(k_x), r, r ] : [ r, __compartment_size(k_y), r ];
          cube(cube_rotated);
      ...
      mirrorpt =
        __component_shape_rotated_90() ? [ 0, __compartment_size(k_y) / 2, 0 ] : [ __compartment_size(k_x) / 2, 0, 0 ];
            ...
            RotateAboutPoint(a = __component_shape_rotated_90() ? 90 : 0, v = [ 0, 0, 1 ], pt = [ 0, 0, 0 ]) {
              {
                ...
                rotate(a = 90, v = [ 1, 0, 0 ])
                {
                  ...
                  rotate(a = __component_is_hex2() ? 30 : __component_is_oct() ? 22.5 : 0, v = [ 0, 0, 1 ])
    ...
    module Helper_MakeLabel(label, x = 0, y = 0) {
      ...
      echo([
        ...
        __label_depth(label),
        str(__label_text(label, x, label_y_reverse)),__is_text(label)
      ]);
    ...
    module MakeLabel(label, x = 0, y = 0) {
      ...
      if (__label_placement_is_center(label)) {
        ...
        children();
        else translate([ __compartment_size(k_x) / 2, -__component_padding(k_y) / 4, __partition_height(k_y) + z_pos ])
        children();
      } else if (__label_placement_is_back(label)) {
        if (__label_placement_is_wall(label))
        ...
        rotate(-90, [ 0, 1, 0 ])
        children();
        else translate([ -__component_padding(k_x) / 4, __compartment_size(k_y) / 2, __partition_height(k_y) + z_pos ])
        children();
      } else if (__label_placement_is_right(label)) {
        if (__label_placement_is_wall(label))
    ...
    module MakeMargin(side) {
      ...
      cube(
        [ m_component_margin_side[k_right], __component_size(k_y), __partition_height(k_x) + m_component_base_height ]);
      else if (side == k_front) cube(
        [ __component_size(k_x), m_component_margin_side[k_front], __partition_height(k_x) + m_component_base_height ]);
      else if (side == k_back) translate([ 0, __component_size(k_y) - m_component_margin_side[k_back], 0 ])
      cube(
        [ __component_size(k_x), m_component_margin_side[k_back], __partition_height(k_x) + m_component_base_height ]);
    }
